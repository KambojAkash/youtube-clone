    ...
    <>
      <div className="w-screen min-h-screen flex items-start flex-col lg:flex-row justify-center gap-5 lg:gap-10 ">
        <div className="w-full p-5 mx-auto lg:w-[50%] lg:h-[400px]">
          ...
          ></iframe>
          <div className="">
            <p className="my-4 text-[12px] lg:text-xl">
              {mainVideoDetail[0]?.snippet?.title}
            </p>
            <div className="flex flex-row gap-5 justify-between ">
              <div className="flex justify-center items-center gap-8 ">
                <div className="flex justify-center items-center">
                  ...
                  <span className="text-[10px] flex lg:text-xl ml-2">
                    {mainVideoDetail[0]?.snippet?.channelTitle}
                  </span>
                </div>
                <button className="h-fit text-[10px]  py-2 px-5 text-black rounded-full bg-white hover:bg-[#ddd]">
                  Subscribe
                </button>
              </div>
                ...
                <div className="w- bg-[rgba(0,0,0,0.6)] py-2 rounded-full flex justify-center items-center">
                  <button className="w-fit rounded-l-full px-4  border-r border-gray-100 flex">
                    ...
                    <span className="text-[12px] lg:text-xl">
                      {useViewFormater(
                        mainVideoDetail[0]?.statistics.viewCount
                      )}
                    </span>
                  </button>
            ...
            <div className="flex justify-start gap-5 mt-3 lg:hidden">
              <div className="w-fit bg-[rgba(0,0,0,0.6)] py-2 rounded-full flex justify-center items-center">
                <button className="w-fit rounded-l-full px-4  border-r border-gray-100 flex">
                  <MdOutlineThumbUp className="text-xl" />
                  <span className="text-[12px] lg:text-xl">
                    {useViewFormater(mainVideoDetail[0]?.statistics.viewCount)}
                  </span>
                </button>
                <button className=" w-fit rounded-r-full px-4 ">
                  <MdOutlineThumbDownOffAlt className="text-xl" />
          ...
          <div className="w-full mt-5 ">
            <h3 className="text-2xl font-bold">
              {languageParts.comment[language]} - {useViewFormater(mainVideoDetail[0]?.statistics?.commentCount)}
            </h3>
            {mainVideoDetail[0]?.statistics?.commentCount>0 && comments?.map((com, i) => {
              return (
                <CommentsSection key={i} user={com.user} text={com.text} />
              );
            })}
          </div>
        </div>
        ...
        <div className="w-full lg:w-[35%]">
          {mainVideoDetail[0]?.snippet?.liveBroadcastContent!="none"&&<LiveChatBox />}
          <h4 className="text-2xl mt-5 text-white font-bold">{languageParts.suggestedVideos[language]}</h4>
